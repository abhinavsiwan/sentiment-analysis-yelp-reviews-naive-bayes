...
def yelp_classify():
    with open("yelp_model.txt") as data_file:
    ...
    with open("dev_yelp_processed_review.json") as data_file:
        reviews_list = json.load(data_file)

    for restaurant_review in reviews_list:
        words = restaurant_review['text'].strip().split()
        ...
        if restaurant_review['stars'] <= 2:
            negative_review_count += 1
            if prob_negative_review > prob_positive_review:
                negative_classified_true += 1
                fout.writelines("Negative " + restaurant_review['review_id'] + "\n")
            else:
                negative_classified_false += 1
                fout.writelines("Positive " + restaurant_review['review_id'] + "\n")
        else:
